...
epatterns = []
epattern1 = '([A-Za-z.]+)\s*@\s*([A-Za-z.]+)\.edu'
epattern2 = '([A-Za-z.]+)\s@\s([A-Za-z.]+)\.edu'
epattern3 = '(\w+)(\s(?:at|@)\s)(\w+)((?: dot |.))(\w+)((?: dot |.))(edu)'
epattern4 = '(\w+)(\s*@*\s*)(\w+)(.)(\w+)(.)(\w+)'
epattern5 = '(\w+(?:.|\s)\w+\s)\(followed by (?:"|&ldquo;)@(\w+.\w+).edu'
epattern6 = '(\w+)\sat\s(\w+\s(?:dt|dot)\s(?:edu|com))'
epattern7 = '(\w+)(?:\s<at symbol>\s|<del>@)(\w+.\w+).edu'
epattern8 = '(\w+)(?: at | AT )(\w+(?: DOT edu|.\w+.EDU))'
epattern9 = '(\w+)&#x40;(\w+.\w+).edu'
epattern10 = '(\w+) WHERE (\w+) DOM edu'
epattern11 = '([\w-]+)@-([-\w]+).-e-d-u'
#epattern12 = '<script> obfuscate\(\'(\w+).edu\',\'(\w+)\'\); <\/script>'
epattern12 = '(\w+)\sat <!-- die!--> (\w+) <!-- spam pigs!--> dot <!-- die!--> edu'

epatterns.append(epattern1)
epatterns.append(epattern2)
epatterns.append(epattern3)
epatterns.append(epattern5)
epatterns.append(epattern6)
epatterns.append(epattern7)
epatterns.append(epattern8)
epatterns.append(epattern9)
epatterns.append(epattern10)
epatterns.append(epattern11)
epatterns.append(epattern12)
#epatterns.append(epattern12)
...
ppatterns.append(pattern1)
ppatterns.append(pattern2)
ppatterns.append(pattern3)
    ...
    for line in f:
        ...
        for epat in epatterns:
            ...
            for m in matches:
                ...
                if m[0]!='Server':
                    if m[1].strip() in ['at','@'] and m[3].strip().lower() in['dot','.',';',''] and m[5].strip().lower() in ['dot','.',';',''] and m[6].strip().lower()=="edu":
                        m= (m[0],m[2],m[4])
                        email = '%s@%s.%s.edu' % m
                    elif len(m)==2:
                        temp= m[1].lower().replace('dt','.')
                        temp= temp.replace('dot','.')
                        temp= temp.replace(' ','')
                        if temp.find('.com')!=-1 or temp.find('edu')!=-1:
                            m = (m[0].strip(),temp.lower())
                            email = '%s@%s' % m
                        elif str(m).find('-')!=-1:
                            m=(str(m[0]).replace('-',''),str(m[1]).replace('-',''))
                            email = '%s@%s.edu' % m
                        # elif str(m[0]).find('obfuscate')!=-1:
                        #     print m
                        #     temp = m[0].replace('obfuscate (','')
                        #     m=(m[1],m[0])
                        #     email = '%s@%s.edu' % m
                        else:
                            m = (m[0].strip(),temp)
